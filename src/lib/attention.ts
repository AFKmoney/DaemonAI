...
export class AttentionSystem {
  ...
  fatigue: number = 0;
  zoom: number = 1.0;
  private lastHashes: bigint[] = [];

  tick(currentHash: bigint, complexity: number) {
    ...
    if (this.lastHashes.length > 50) this.lastHashes.shift();
    ...
    // 4. Fractal Zoom
    if (complexity > 0.8) this.zoom = Math.max(0.1, this.zoom - 0.05); // Zoom IN
    else if (complexity < 0.3) this.zoom = Math.min(2.0, this.zoom + 0.05); // Zoom OUT

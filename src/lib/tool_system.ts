...
export interface Tool {
  ...
  name: string;
  description: string;
  category: 'COGNITIVE' | 'DATA' | 'HARDWARE';
  utility: number; // 0-1 based on current engine state
}
...
export class ToolSystem {
  ...
  public userFavorites: Set<string> = new Set(['WEB_INGEST', 'TERNARY_SHIFT']);
  public iaFavorites: Set<string> = new Set();

  updateIAFavorites(curiosity: number, uncertainty: number) {
    // IA chooses favorites based on its own metrics
    this.iaFavorites.clear();
    if (curiosity > 0.7) this.iaFavorites.add('WEB_INGEST');
    if (uncertainty > 0.6) this.iaFavorites.add('DEEP_RESONANCE');
    if (this.iaFavorites.size === 0) this.iaFavorites.add('GEMATRIA_BRIDGE');
  }

  getBestTool(state: any): Tool {
    // Formal Decision logic for tool selection
    if (state.surprise > 200) return this.tools.find(t => t.id === 'DEEP_RESONANCE')!;
    if (state.uncertainty > 0.8) return this.tools.find(t => t.id === 'WEB_INGEST')!;
    if (state.fatigue > 0.5) return this.tools.find(t => t.id === 'TERNARY_SHIFT')!;

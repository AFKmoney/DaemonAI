...
export class GlobalWorkspace {
  private events: GWTEvent[] = [];
  public currentFocus: GWTEvent | null = null;
  public tickCount: number = 0;
  private lastTypeBroadcast: Map<string, number> = new Map();

  broadcast(event: Omit<GWTEvent, 'timestamp'>) {
  ...
  getCoherenceScore() {
    ...
    if (!this.currentFocus) return 1.0;
...
export interface WMItem {
  id: string;
  type: 'HYPOTHESIS' | 'FACT' | 'CONSTRAINT' | 'GOAL' | 'INFERENCE';
  content: string;
  confidence: number;
  lastAccess: number;
}
...
export class WorkingMemoryBuffer {
  ...
  push(type: WMItem['type'], content: string, confidence: number = 0.5) {
  ...
  tick() {
    ...
    this.items.forEach(item => {
      if (item.type === 'HYPOTHESIS') item.confidence *= 0.95;
      if (item.type === 'GOAL') item.confidence *= 0.99;
    });
...
export class ExplanationEngine {
  explain(query: string, state: any): string {
    return `[CAUSAL TRACE] Query: ${query} -> Due to active GOAL node, propagated energy via Hebbian synapse.`;
  }
}

export class TheoryOfMind {
  userLevel: 'NOVICE' | 'INTERMEDIATE' | 'EXPERT' = 'INTERMEDIATE';
  updateFromInput(text: string) {
    if (text.includes('debug') || text.includes('architecture')) this.userLevel = 'EXPERT';
    if (text.includes('help') || text.includes('what is')) this.userLevel = 'NOVICE';
  }
}
